# Aries Chain Custom EIPs

This document explains how Aries Chain defines custom EIPs to modify the behavior of EVM opcodes.
...
Custom EIPs should be defined in an `eips` package inside the `./app/eips/` folder. This organization of custom implementations is not a strict requirement, but is the suggested approach to
...
where `vm` is the package `"github.com/arieschainorg/Aries-Chain/x/evm/core/vm"`.
...
It is possible to modify any of the fields defined in the type via the `operation` setter
methods:
...
[`x/evm/`](https://github.com/arieschainorg/Aries-Chain/tree/main/x/evm) folder.
...
Custom activators should be defined in a structure with the same data type, like in the example below:

```go
// Activate custom EIPs: 0000, 0001, 0002, etc
ariesActivators = map[int]func(*vm.JumpTable){
...
custom functionalities that can be activated. To specify which custom EIP activate, we should modify the `x/evm` module params. The parameter orchestrating enabled custom EIPs is
`DefaultExtraEIPs`, and Aries Chain provides an easy and safe way to customize it.
...
It is important to notice that the configurator will only allow to append new entries to the default ones. The reason behind this choice is to ensure the correct and safe execution of the virtual machine but still
...
Earlier iterations of this EVM implementation used string identifiers such as `"chain_0"` for custom EIPs.
These were converted to integer identifiers to reduce the maintenance burden of integrating with upstream
go-ethereum.
