...
# 51,000 ARES validator staking limits, and low-latency block timeouts.
# ==============================================================================
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  echo "$VAL_MNEMONIC" | ariesd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  echo "$VAL_MNEMONIC" | ariesd init "$MONIKER" -o --chain-id "$CHAINID" --home "$CHAINDIR" --recover
  ...
  # Fund the validator key with 100,000,000 ARES (10^26 aares) to easily cover the 51,000 ARES self-stake rule
  ariesd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000aares --keyring-backend "$KEYRING" --home "$CHAINDIR"

  # Prefund all default developer accounts in genesis
  for ((i=0; i<${#default_mnemonics[@]}; i++)); do
    keyname="dev${i}"
    mnemonic="${default_mnemonics[i]}"
    echo "$mnemonic" | ariesd keys add "$keyname" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
    add_genesis_funds "$keyname"
  done

  # ---------------- Create GenTx satisfying the 51,000 ARES Validator Staking limit ----------------
  # Staking 51,000 ARES (5.1 * 10^22 aares) to register our devnet validator
  echo "🗳️  Creating Genesis delegation transaction of 51,000 ARES..."
  ariesd genesis gentx "$VAL_KEY" 51000000000000000000000aares --gas-prices ${BASEFEE}aares --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
  ...
  echo "⚡ Optimizing consensus parameters (300ms optimistic execution, ~1s economic finality targets)..."
  ...
  sed -i.bak 's/create_empty_blocks_interval = "0s"/create_empty_blocks_interval = "5m"/g' "$CONFIG_TOML"
...
echo "Validator:    $VAL_KEY (51,000 ARES active stake)"
echo "EVM RPC Port: http://127.0.0.1:8545"
echo "=============================================================================="

# Start node execution daemon
ariesd start \
  --pruning nothing \
  --log_level $LOGLEVEL \
  --minimum-gas-prices=0aares \
  --evm.min-tip=0 \
  --home "$CHAINDIR" \
