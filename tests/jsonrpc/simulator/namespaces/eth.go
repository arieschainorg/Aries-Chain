...
import (
	...
	"github.com/arieschainorg/Aries-Chain/tests/jsonrpc/simulator/contracts"
...
// Uncle methods - these should always return 0 or nil in Aries Chain (no uncles in PoS)
func EthGetUncleCountByBlockHash(rCtx *types.RPCContext) (*types.RpcResult, error) {
	...
	// Should always be 0 in Aries Chain
	result := &types.RpcResult{
...
func EthGetUncleByBlockHashAndIndex(rCtx *types.RPCContext) (*types.RpcResult, error) {
	...
	// Should always be nil in Aries Chain
	result := &types.RpcResult{
...
// Aries Chain and go-ethereum behaviour for plain value transfers.
	...
	if err != nil {
		if err.Error() == "the method "+string(MethodNameEthFeeHistory)+" does not exist/is not available" ||
			...
			return &types.RpcResult{
				...
				ErrMsg:   "Method not implemented in Aries Chain",
				Category: NamespaceEth,
			}, nil
		}
		return &types.RpcResult{
	...
	if err != nil {
		if strings.Contains(err.Error(), "does not exist/is not available") ||
			...
			result := &types.RpcResult{
				...
				Status:   types.NotImplemented,
				ErrMsg:   "Method not implemented in Aries Chain - eth_simulateV1 is a newer Ethereum API",
				Category: NamespaceEth,
			}
			return result, nil
		}
	...
	if err != nil {
		if strings.Contains(err.Error(), "does not exist/is not available") ||
			...
			result := &types.RpcResult{
				...
				Status:   types.NotImplemented,
				ErrMsg:   "Method not implemented in Aries Chain - use eth_getPendingTransactions instead",
				Category: NamespaceEth,
			}
			return result, nil
		}
